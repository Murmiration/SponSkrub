...
ClipTime[] get_video_skip_times_direct(string video_id, Categories[] categories, string api_url, string proxy="") {
	auto data = proxy_get("http://%s/api/skipSegments?videoID=%s&categories=%s".format(api_url, video_id, `["`~(cast(string[])categories).join(`",%20"`)~`"]`), proxy);
...
ClipTime[] get_video_skip_times_private(string video_id, Categories[] categories, string api_url, string proxy="") {
	auto data = proxy_get("http://%s/api/skipSegments/%s?categories=%s".format(api_url, sha256Of(video_id).toHexString!(LetterCase.lower)[0..uniform(3,32)], `["`~(cast(string[])categories).join(`",%20"`)~`"]`), proxy);
	auto json = parseJSON(data);
